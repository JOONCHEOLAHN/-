...
for i in [0]:  #스크래핑 할 페이지수
    ...
    if i == 0:
          # for a, b, c, d in zip(a_num_list, a_title_list[7:], a_writer_list[7:], a_regdate_list[7:]):
          for a, b in zip( a_title_list[7:], a_writer_list[7:]):  # 나눔해요게시판, 목격담, 5세대 시공 장착 정비 수리
               list = []
         ...
         for a, b in zip(a_title_list, a_writer_list):
               list = []
               list.append(a.text.strip())
               list.append(b.text)
               # list.append(a.text)
